...
if it exists. The tail node of the list will set it's next pointer to 
the index-th node. If index = -1, then the tail node points to null and no cycle exists.
...
class Solution:
    def hasCycle(self, head: Optional[ListNode]) -> bool:
        # Determining if a linked list has a cycle using Floyd's Tortoise and Hare algorithm. 
        ...
        while fast and fast.next:
           slow = slow.next         # move pointer one step
           fast = fast.next.next    # move pointer two step

            if slow == fast:        # if they meet, cycle exits
                return True
