...
class Solution:
    def isSameTree(self, p: Optional[TreeNode], q: Optional[TreeNode]) -> bool:
        # if both lists are empty, they are same return true
        if not p and not q:
            return True

        # if one is tree is empty and other not return false
        if not p or not q:
            return False

        # if current node has different values, they are not the same return false
        if p.val != q.val:
            return False
